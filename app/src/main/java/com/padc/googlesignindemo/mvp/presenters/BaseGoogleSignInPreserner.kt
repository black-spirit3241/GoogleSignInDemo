...
import com.padc.firebaseauthenication.mvp.views.BaseGoogleSignInView
import com.padc.googlesignindemo.R

abstract class BaseGoogleSignInPresenter<V:BaseGoogleSignInView> : BasePresenter<V>(){

    private val userModel : GoogleAuthenicationModel = GoogleAuthenicationModelImpl
    ...
    fun googleSingIn(context: Context){
//         Configure Google Sign In
    ...
    fun onActivityResult(requestCode: Int, resultCode: Int, data: Intent?, owner: LifecycleOwner){
        userModel.callbackManager.onActivityResult(requestCode,requestCode,data)
        if(requestCode == RC_GOOGLE_SIGN){
            val task=GoogleSignIn.getSignedInAccountFromIntent(data)
            try {
                // Google Sign In was successful, authenticate with Firebase
                val account = task.getResult(ApiException::class.java)
                userModel.firebaseAuthWithGoogle(account!!){
